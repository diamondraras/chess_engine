...
class King(Piece):
        ...
        for i, piece in enumerate(player.opponent.pieces):
            if piece.typeOf() != "Pawn" and piece.typeOf() != "King":
                dangerous_positions = dangerous_positions + \
                    piece.getPossiblesMoves(player.opponent)
            if piece.typeOf() == "Pawn":
                dangerous_positions += piece.getCapturablePositions(
                    player.opponent)
    ...
    def getPossiblesMoves(self, player):
        ...
        king_opponent_capabilites = king_opponent.getCapabilities(
            player.opponent)
        king_opponent_dangerous_positions = king_opponent.getDangerousPositions(
            player.opponent)
