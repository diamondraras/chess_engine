...
class Pawn(Piece):
    ...
    def checkPromotion(self, player):
        ...
        if self.positions.y == y:
            player.pieces[player.getPieceIndex(
                self.positions)] = Queen(self.positions.x, y)
    ...
    def getCapturablePositions(self, player):
        ...
        # Capture left move
        try:
            left_capture_move = Position(
                self.positions.x - 1, self.positions.y + (direction * 1))
    ...
    def getPossiblesMoves(self, player):
        ...
        # Normal move
        try:
            normal_move = Position(
                self.positions.x, self.positions.y + (direction * 1))
        ...
        try:
            first_move = Position(
                self.positions.x, self.positions.y + (direction * 2))
        ...
        # En passant Left
        try:
            left_en_passant_opponent = Position(
                self.positions.x - 1, self.positions.y)
            left_en_passant_destination = Position(
                self.positions.x - 1, self.positions.y + (direction))
        ...
        # En passant Right
        try:
            right_en_passant_opponent = Position(
                self.positions.x + 1, self.positions.y)
            right_en_passant_destination = Position(
                self.positions.x + 1, self.positions.y + (direction))
